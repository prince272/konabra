...
func NewCategoryHandler(router *gin.Engine, categoryService *services.CategoryService, jwtHelper *helpers.JwtHelper) *CategoryHandler {
	...
	{
		categoryGroup.GET("/", handler.handle(handler.GetPaginatedCategories))
		categoryGroup.GET("/all", handler.handle(handler.GetCategories))
		categoryGroup.GET("/{id}", handler.handle(handler.GetCategoryById))
	}
...
func (handler *CategoryHandler) handle(handlerFunc func(*gin.Context) (any, *problems.Problem)) gin.HandlerFunc {
